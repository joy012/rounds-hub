...
function parseBedFromJson(value: JsonValue): Bed | null {
  ...
  if (
    ...
    if (
      ...
      const dxObj =
        p.dx && typeof p.dx === 'object' && !Array.isArray(p.dx) ? (p.dx as JsonObject) : null;
      const planObj =
        p.plan && typeof p.plan === 'object' && !Array.isArray(p.plan)
          ? (p.plan as JsonObject)
          : null;
      const invArr = Array.isArray(p.inv)
        ? (p.inv as JsonValue[]).filter(
            (r): r is JsonObject => isJsonObject(r) && typeof r.id === 'string'
          )
        : [];
      const invRows: InvRow[] = invArr.map(
        (r): InvRow => ({
          id: r.id as string,
          ...
          investigationImage:
            typeof r.investigationImage === 'string' ? r.investigationImage : undefined,
      ...
      bed.patient = {
        ...
        ...(dxObj && {
          dx: {
            ...
            image: typeof dxObj.image === 'string' ? dxObj.image : undefined,
          },
        }),
        ...(planObj && {
          plan: {
            text: typeof planObj.text === 'string' ? planObj.text : undefined,
            image: typeof planObj.image === 'string' ? planObj.image : undefined,
          },
        }),
...
function parseWardFromJson(raw: string): Ward | null {
  ...
  if (wardNumber !== undefined && wardNumber !== null && typeof wardNumber !== 'string')
    return null;
  const beds: Bed[] = [];
